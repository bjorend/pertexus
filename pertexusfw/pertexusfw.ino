...
#define _GP_PORTEXP_WRITE(pstatep, setnum, bitnum, val)                  \
    do {                                                                \
        ...
        if ((val) != 0) {                                               \
            /* Turning bit on */                                        \
            (pstatep)->pexpregs[(setnum)] |= (1 << (bitnum));           \
        }                                                               \
        else {                                                          \
            /* Turning bit off */                                       \
            (pstatep)->pexpregs[(setnum)] &= ~(1 << (bitnum));          \
        ...
        portexp_write((pstatep),                                        \
                      PCA9535_CMD_OUTP0 + (setnum),                     \
                      (pstatep)->pexpregs[(setnum)]);                   \
    } while (FALSE)
...
#define PERTEXUS_STATE_OPSTATE_ROVERRUN       12
    ...
    uint8_t         pexpregs[3 * PCA9535_NPORTS];
...
cmd_gpio(cmdif_t *cmdifp, int argc, char **argv)
{
    ...
    bool_t          asinputs, goingup, paused, forcemode, init;
    pertexus_state_t    *pstatep;
    Stream          *streamp;
    ...
    if (argc > 1) {
        if (strcmp_P(argv[1], PSTR("-forcein")) == 0) {
            ...
            streamp->println(
                F("Usage: gpio [-init|[-out|-forcein|-forceout]]\r\n"));
            return;
        }
    }

    if (init && (forcemode || !asinputs)) {
        streamp->println(F("-init must be used by itself"));
        return;
    }
    ...
    for (idx = 0; idx < PCA9535_NPORTS; idx++) {
        ...
        streamp->print(idx);
        streamp->print(F("  0x"));
        hexprint(streamp, pstatep->pexpregs[PCA9535_CMD_OUTP0 + idx
                                           - PCA9535_CMD_OUTP0], 2);
        streamp->println();
        ...
        streamp->print(idx);
        streamp->print(F("   0x"));
        hexprint(streamp, pstatep->pexpregs[PCA9535_CMD_INV0 + idx
                                           - PCA9535_CMD_OUTP0], 2);
        streamp->println();
        ...
        streamp->print(idx);
        streamp->print(F("   0x"));
        hexprint(streamp, pstatep->pexpregs[PCA9535_CMD_CFG0 + idx
                                           - PCA9535_CMD_OUTP0], 2);
        streamp->println();
    }
...
cmd_sd(cmdif_t *cmdifp, int argc, char **argv)
{
    ...
    if (argc < 2) {
usage:
        streamp->println(F("Usage: sd init|dir|read"));
        return;
    }
    ...
    if (strcmp_P(argv[1], PSTR("init")) == 0) {
        ...
        readfile = pertexus_state.sdp->open(argv[2], FILE_READ);
